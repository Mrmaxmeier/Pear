...
fn get_symbol(num: usize) -> Symbol {
    ...
    Symbol::intern(&chars[start..(start + need)])
}
...
fn remonad_param(ecx: &ExtCtxt, param: P<Expr>, stmts: &mut Vec<Stmt>) -> P<Expr> {
    ...
    match param_expr.node {
        ExprKind::Call(..) | ExprKind::MethodCall(..) | ExprKind::Mac(..) => {
            let unique_symbol = get_symbol(stmts.len()); // FIXME: Generate this.
            let unique_ident = Ident::new(unique_symbol, param.span);
